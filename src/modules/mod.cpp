...
private:
	 ...
	 * Deflemask. SampleMapper handles the details of this.
	 */
	class SampleMapper;

	using SampleMap = std::map<SoundIndexType<DMF>, SampleMapper>;
	...
	using PriorityEffect = std::pair<EffectPriority, d2m::Effect>;
	...
	std::shared_ptr<const MODConversionOptions> options_;

	// Whether to use an order at the start of the module to set up the initial tempo and other stuff
	static constexpr bool kUsingSetupOrder = true;
};
...
static auto GenerateWavetableSample(uint32_t* wavetable_data, unsigned length) -> std::vector<std::int8_t>;
...
void MOD::ConvertImpl(const ModulePtr& input)
{
	...
	switch (input->GetType())
	{
		...
		{
			DMFConverter converter{*this, *input->Cast<const DMF>()};
			converter.Convert();
			break;
		}
...
class MOD::DMFConverter::SampleMapper
{
public:
	...
	auto Init(SoundIndexType<DMF> dmf_sound_index, SoundIndexType<MOD> starting_sound_index, std::pair<Note, Note> dmf_note_range) -> SoundIndexType<MOD>;
...
private:
	...
	int num_mod_samples_ = 0;
	SampleType sample_type_ = SampleType::kSilence;
	bool downsampling_needed_ = false;
	int mod_octave_shift_ = 0;
};

auto MOD::DMFConverter::SampleMapper::Init(SoundIndexType<DMF> dmf_sound_index, SoundIndexType<MOD> starting_sound_index, std::pair<Note, Note> dmf_note_range) -> SoundIndexType<MOD>
{
	...
	const auto lowest_note = dmf_note_range.first;
	const auto highest_note = dmf_note_range.second;

	// Note ranges always start on a C, so get nearest C note (in downwards direction):
	const auto lowest_note_nearest_c = Note{NotePitch::kC, lowest_note.octave}; // DMF note

	// Get the number of MOD samples needed
	const int range = GetNoteRange(lowest_note_nearest_c, highest_note);
	if (range <= 36) { num_mod_samples_ = 1; }
	else if (range <= 72) { num_mod_samples_ = 2; }
	else { num_mod_samples_ = 3; }
	...
	{
		range_start_.emplace_back(NotePitch::kC, 0);
		range_start_.emplace_back(NotePitch::kC, 2);
		range_start_.emplace_back(NotePitch::kC, 5);
	...
	if (num_mod_samples_ == 2) { current_high_end.octave += 3; }
	...
	*/
	...
	auto octave_to_sample_length_map = std::array{256u, 128u, 64u, 32u, 16u, 8u};
	...
	// For whatever reason, wave samples need to be transposed down one octave to match their sound in Deflemask
	if (sample_type_ == SampleType::kWave) { mod_sample_lengths_[0] *= 2; }
	...
	{
		range_start_.emplace_back(NotePitch::kC, std::uint8_t(lowest_possible_range_start.octave + 3));
		mod_sample_lengths_[1] = octave_to_sample_length_map[range_start_[1].octave];

		// For whatever reason, wave samples need to be transposed down one octave to match their sound in Deflemask
		if (sample_type_ == SampleType::kWave) { mod_sample_lengths_[1] *= 2; }
...
auto MOD::DMFConverter::SampleMapper::InitSilence() -> SoundIndexType<MOD>
{
...
auto MOD::DMFConverter::SampleMapper::GetMODNote(const Note& dmf_note, NoteRange& mod_note_range) const -> Note
{
	...
	if (sample_type_ == SampleType::kSilence) { return mod_note; }
...
auto MOD::DMFConverter::SampleMapper::GetMODNoteRange(const Note& dmf_note) const -> NoteRange
{
	...
	if (num_mod_samples_ == 1) { return NoteRange::kFirst; } // The only option

	const std::uint16_t& octave_of_nearest_c = dmf_note.octave;
...
auto MOD::DMFConverter::SampleMapper::GetMODSampleId(const Note& dmf_note) const -> SoundIndexType<MOD>
{
...
auto MOD::DMFConverter::SampleMapper::GetMODSampleId(NoteRange mod_note_range) const -> SoundIndexType<MOD>
{
...
auto MOD::DMFConverter::SampleMapper::GetMODSampleLength(NoteRange mod_note_range) const -> unsigned
{
...
auto MOD::DMFConverter::SampleMapper::GetMODNoteRange(SoundIndexType<MOD> mod_sound_index) const -> NoteRange
{
...
auto MOD::DMFConverter::SampleMapper::GetMODNoteRangeName(NoteRange mod_note_range) const -> NoteRangeName
{
	// Gets NoteRange which can be used for printing purposes
	switch (mod_note_range)
	{
		case NoteRange::kFirst:
			if (num_mod_samples_ == 1) { return NoteRangeName::kNone; }
			return NoteRangeName::kLow;
		case NoteRange::kSecond:
			if (num_mod_samples_ == 2) { return NoteRangeName::kHigh; }
...
void MOD::DMFConverter::Convert()
{
	const bool verbose = GlobalOptions::Get().GetOption(GlobalOptions::OptionEnum::kVerbose).GetValue<bool>();

	if (verbose) { std::cout << "Starting to convert to MOD...\n"; }

	if (dmf_.GetSystem().type != DMF::SystemType::kGameBoy) // If it's not a Game Boy
	{
		throw MODException(ModuleException::Category::kConvert, MOD::ConvertError::kNotGameBoy);
	}

	const auto& dmf_data = dmf_.GetData();
	auto& mod_data = mod_.GetData();
	...
	const OrderIndex num_orders = dmf_gen_data->GetNumOrders().value() + (OrderIndex)kUsingSetupOrder;
	...
	// Don't pad with spaces b/c exporting to WAV in ProTracker will keep those spaces in the exported file name
	if (mod_global_data.title.size() > 20) { mod_global_data.title.resize(20); }
	...
	if (verbose) { std::cout << "Converting samples...\n"; }

	SampleMap sample_map;
	ConvertSamples(sample_map);
	assert(sample_map.find(SoundIndex<DMF>::None{}) == sample_map.end() || sample_map.at(SoundIndex<DMF>::None{}).GetFirstMODSampleId() == 1);

	///////////////// CONVERT PATTERN DATA

	if (verbose) { std::cout << "Converting pattern data...\n"; }

	ConvertPatterns(sample_map);

	///////////////// CLEAN UP

	if (verbose) { std::cout << "Done converting to MOD.\n\n"; }
}

void MOD::DMFConverter::ConvertSamples(SampleMap& sample_map)
{
	...
	const auto& dmf_sound_indexes = dmf_.GetGeneratedData()->Get<GeneratedData<DMF>::kSoundIndexesUsed>().value();
	const auto& dmf_sound_index_note_extremes = dmf_.GetGeneratedData()->Get<GeneratedData<DMF>::kSoundIndexNoteExtremes>().value();
	...
	if (dmf_.GetGeneratedData()->Get<GeneratedData<DMF>::kNoteOffUsed>().value())
	{
	...
	for (const auto& dmf_sound_index : dmf_sound_indexes)
	{
		...
		{
			mod_.status_.AddWarning(GetWarningMessage(MOD::ConvertWarning::kWaveDownsample, std::to_string(std::get<SoundIndex<DMF>::Wave>(dmf_sound_index).id)));
		}
	}

	mod_.total_mod_samples_ = mod_current_sound_index - 1; // Set the number of MOD samples that will be needed. (minus sample #0 which is special)

	// TODO: Check if there are too many samples needed here, and throw exception if so

	ConvertSampleData(sample_map);
}

void MOD::DMFConverter::ConvertSampleData(const SampleMap& sample_map)
{
	...
	for (const auto& [dmf_sound_index, sample_mapper] : sample_map)
	{
		...
		{
			auto note_range = static_cast<NoteRange>(note_range_int);
			...
			switch (sample_mapper.GetSampleType())
			{
				case SampleType::kSilence:
					si.name = "Silence";
					si.volume = 0;
					si.data = std::vector<std::int8_t>(si.length, 0);
					break;
			...
			if (si.id == 1) // #0 is a special value, not the 1st MOD sample
			{
				...
				si.name = mod_.GetAuthor();
			}

			if (si.name.size() > 22) { throw std::invalid_argument("Sample name must be 22 characters or less"); }

			// Pad name with zeros
			while (si.name.size() < 22) { si.name += " "; }

			mod_.samples_[si.id] = si;
		}
	}
}

void MOD::DMFConverter::ConvertPatterns(const SampleMap& sample_map)
{
	auto& mod_data = mod_.GetData();

	unsigned initial_tempo, initial_speed; // Together these will set the initial BPM
	ConvertInitialBPM(initial_tempo, initial_speed);
	...
	if (kUsingSetupOrder)
	{
		...
		// Set initial speed
		if (options_->GetTempoType() != MODConversionOptions::TempoType::kEffectCompatibility)
		{
			Row<MOD> speed_row;
			speed_row.sample = 0;
	...
	const OrderIndex dmf_num_orders = dmf_.GetGeneratedData()->GetNumOrders().value();
	const RowIndex dmf_num_rows = dmf_.GetData().GetNumRows();

	auto state_readers = dmf_.GetGeneratedData()->GetState().value().GetReaders();
	auto& global_reader = state_readers.global_reader;
	auto& channel_readers = state_readers.channel_readers;

	// Extra state info needed:
	std::array<NoteRange, 4> note_range;
	note_range.fill(NoteRange::kFirst); // ???
	...
	for (OrderIndex dmf_order = 0; dmf_order < dmf_num_orders; ++dmf_order)
	{
		...
		for (RowIndex dmf_row = 0; dmf_row < dmf_num_rows; ++dmf_row)
		{
			...
			{
				global_effects.push_back({ kEffectPriorityStructureRelated, { Effects::kPosJump, static_cast<EffectValue>(global_reader.GetOneShot<GlobalState<DMF>::kPosJump>() + kUsingSetupOrder) } });
			}
			...
			for (ChannelIndex channel = 0; channel < mod_data.GetNumChannels(); ++channel)
			{
				...
				if (channel != dmf::GameBoyChannel::kNoise)
				{
					...
					mod_effects[channel] = ConvertEffects(channel_reader);
					mod_row_data[channel] = ConvertNote(channel_reader, note_range[channel], set_sample[channel], set_volume_if_not[channel], sample_map, mod_effects[channel]);
				}
			}
			...
			for (ChannelIndex channel = 0; channel < mod_data.GetNumChannels(); ++channel)
			{
				mod_data.SetRow(channel, dmf_order + kUsingSetupOrder, dmf_row, mod_row_data[channel]);
			}
		}
		...
		for (RowIndex dmf_row = dmf_num_rows; dmf_row < 64; ++dmf_row)
		{
			for (ChannelIndex channel = 0; channel < mod_data.GetNumChannels(); ++channel)
			{
				...
				mod_data.SetRow(channel, dmf_order + (int)kUsingSetupOrder, dmf_row, temp_row_data);
			}
		}
	}
}
...
auto MOD::DMFConverter::ConvertEffects(ChannelStateReader<DMF>& state) -> PriorityEffect
{
	...
	{
		const auto effect_value = static_cast<EffectValue>(val.value);
		switch (val.type)
		{
			case PortamentoStateData::kUp:
				if (options_->AllowPortamento()) { return { kEffectPriorityPortUp, { Effects::kPortUp, effect_value } }; }
				break;
			case PortamentoStateData::kDown:
				if (options_->AllowPortamento()) { return { kEffectPriorityPortDown, { Effects::kPortDown, effect_value } }; }
				break;
			case PortamentoStateData::kToNote:
				if (options_->AllowPort2Note()) { return { kEffectPriorityPort2Note, { Effects::kPort2Note, effect_value } }; }
				break;
			default:
				assert(0);
				break;
	...
	if (state.GetDelta(ChannelState<DMF>::kVolume))
	{
		...
		const auto mod_volume = (uint8_t)std::round(dmf_volume / (double)dmf::kGameBoyVolumeMax * (double)kVolumeMax); // Convert DMF volume to MOD volume

		return { kEffectPriorityVolumeChange, { mod::Effects::kSetVolume, mod_volume } };
	}

	// Arpeggios
	if (EffectValue val = state.Get<ChannelState<DMF>::kArp>(); val > 0 && options_->AllowArpeggio())
	{
		return { kEffectPriorityArp, { Effects::kArp, val } };
	}

	// Vibrato
	if (EffectValue val = state.Get<ChannelState<DMF>::kVibrato>(); val > 0 && options_->AllowVibrato())
	{
...
auto MOD::DMFConverter::ConvertNote(ChannelStateReader<DMF>& state, NoteRange& note_range, bool& set_sample, int& set_vol_if_not, const SampleMap& sample_map, PriorityEffect& mod_effect) -> Row<MOD>
{
	...
	switch (dmf_note.index())
	{
		...
		case NoteTypes::kNote:
		{
			...
			const auto& sample_mapper = sample_map.at(dmf_sound_index);

			NoteRange new_note_range;
...
void MOD::DMFConverter::ApplyEffects(std::array<Row<MOD>, 4>& row_data, const std::array<PriorityEffect, 4>& mod_effects, std::vector<PriorityEffect>& global_effects)
{
	...
	using ChannelPriorityEffect = std::pair<ChannelIndex, PriorityEffect>;
...
void MOD::DMFConverter::ConvertInitialBPM(unsigned& tempo, unsigned& speed)
{
	...
	const double desired_bpm = dmf_.GetBPM();
	...
	if (options_->GetTempoType() == MODConversionOptions::TempoType::kEffectCompatibility)
	{
		...
		{
			tempo = 255;
			mod_.status_.AddWarning(GetWarningMessage(ConvertWarning::kTempoHighCompat));
		}
		else if (tempo < 32)
		{
			tempo = 32;
			mod_.status_.AddWarning(GetWarningMessage(ConvertWarning::kTempoLowCompat));
		}
		else if ((desired_bpm * 2.0) - static_cast<double>(tempo) > 1e-3)
		{
			mod_.status_.AddWarning(GetWarningMessage(ConvertWarning::kTempoAccuracy));
		}
		return;
	}
	...
	if (desired_bpm > kHighestBPM)
	{
		tempo = 255;
		speed = 1;
		mod_.status_.AddWarning(GetWarningMessage(ConvertWarning::kTempoHigh));
		return;
	}
	...
	if (desired_bpm < kLowestBPM)
	{
		tempo = 32;
		speed = 31;
		mod_.status_.AddWarning(GetWarningMessage(ConvertWarning::kTempoLow));
		return;
	}
	...
	{
		// Check if it's not even possible with this speed value
		if (3 * 32.0 / d > desired_bpm || desired_bpm > 3 * 255.0 / d) { continue; }
	...
	if (best_BPM_diff > 1e-3) { mod_.status_.AddWarning(GetWarningMessage(ConvertWarning::kTempoAccuracy)); }
}
...
void MOD::ExportImpl(const std::string& filename)
{
	...
	const bool verbose = GlobalOptions::Get().GetOption(GlobalOptions::OptionEnum::kVerbose).GetValue<bool>();
	if (verbose) { std::cout << "Saved MOD file to disk.\n\n"; }
}
...
void MOD::ExportSampleInfo(std::ofstream& fout) const
{
	...
	{
		if (sample.name.size() > 22) { throw std::length_error("Sample name must be 22 characters or less"); }

		// Pad name with spaces
		std::string name_copy = sample.name;
		while (name_copy.size() < 22) { name_copy += " "; }
...
{
	const auto num_orders = static_cast<std::uint8_t>(GetData().GetNumOrders());
	...
	{
		fout.put(static_cast<std::uint8_t>(pattern_id));
	}
	...
	for (const auto& pattern : mod_data.PatternsRef())
	{
		for (RowIndex row = 0; row < mod_data.GetNumRows(); ++row)
		{
			for (ChannelIndex channel = 0; channel < mod_data.GetNumChannels(); ++channel)
			{
				...
				//const std::uint16_t effect = ((uint16_t)rowData.EffectCode << 4) | rowData.EffectValue;
